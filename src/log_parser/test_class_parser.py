...
class TestClass:
    def __init__(self):
        ...
        TestClass类成员变量：
        test_class_name:str
        test_case_list:list             该testclass包含的所有testcase
                                        数组类型
                                        保存testcase对象
        test_case_ptr                   当前parse到的test_case的对象指针
        self.cl_s_redundancy:float      （ClS冗余）testClass相对于testSuite的冗余
                                        由外部计算 并给对象赋值
        probe_dict:dict                 当前testclass的所有probe key为probe_id value=被cover的次数
        """
    ...
    def parse_line(self, line):
        ...
        if len(tmp_list) is 2 and tmp_list[0] == TESTCASE_STR_PREFIX:
            ...
            if self.test_case_ptr is None or test_case_name != self.test_case_ptr.get_name():
                ...
                self.test_case_list.append(self.test_case_ptr)
        # 旧的test_case
